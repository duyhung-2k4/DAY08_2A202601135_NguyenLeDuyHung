...
| **7** | `ImportError: DLL load failed while importing chromadb_rust_bindings: An Application Control policy has blocked this file` ở Task 4 | ChromaDB từ bản 1.0 trở đi dùng native Rust binding. Windows **Smart App Control** (bật sẵn trên nhiều máy Win 11) chặn file `.pyd` chưa được ký. | Chạy PowerShell: `Get-ChildItem "<site-packages>\chromadb_rust_bindings" -Recurse \| Unblock-File`, rồi kiểm tra `python -c "import chromadb_rust_bindings"`. **Không** hạ cấp về `chromadb<1.0`: bản đó cần `chroma-hnswlib` mà thư viện này chưa có wheel cho Python 3.13 và sẽ đòi compiler C++. |
| **8** | Bài crawl ở Task 2 ra hàng chục nghìn ký tự nhưng gần như toàn menu điều hướng | Trang WordPress của các trường có sidebar/menu rất lớn; `raw_markdown` giữ nguyên tất cả. Đo thực tế trên `css.vnu.edu.vn`: ~85% ký tự là link menu. | Dùng `PruningContentFilter` của Crawl4AI và lấy `result.markdown.fit_markdown` thay vì `raw_markdown` (xem `src/task2_crawl_news.py`). Không lọc thì phần lớn chunk sẽ là danh sách link vô nghĩa, kéo tụt cả Context Precision lẫn Faithfulness khi chấm RAGAS. |
| **9** | Task 3 tạo ra file `.md` rỗng (0 ký tự) từ PDF | PDF là **bản scan** (ảnh chụp văn bản), MarkItDown không có OCR nên không trích được text. | Kiểm tra kích thước output; nếu < 200 ký tự thì tìm bản PDF dạng text khác của cùng văn bản. Ví dụ QĐ 3626/QĐ-ĐHQGHN: bản trên `uet.edu.vn` là scan 18 MB (0 ký tự), bản trên `ussh.vnu.edu.vn` là text 643 KB (95k ký tự). |
